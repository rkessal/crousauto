...
<html>
<head>
	<title>Création d'une Carte Essence</title>
...
<h1 class="title">Création d'une Carte Essence</h1>
<br>
	<?php
	if(isset($_POST["Valider"]))
	{
		...
		$conectBDD = new BDD();
		$res = $conectBDD->Carte_Essence_Create($nom, $vehicule, $_POST['fournisseur'], $_POST['dateRenouvellement']);
		$infosCarte = $conectBDD->Carte_Essence_Infos_parNom($nom);
		if($res)
		{
			?>
			<div class="alert alert-success" role="alert">
			  	La Carte Essence à bien été ajouté.
			</div>
	...
	?>
		...
		<div class="input-group" style="text-align: center">
  		...
  		<div class="input-group" style="text-align: center">
  			...
  			<input type="text" name="dateRenouvellement" required style="width: 200px" class="input-text form-control" aria-describedby="basic-addon1"  id="datepicker" autocomplete="off" placeholder="Sélectionnez une date">
  		</div>
  		<br>
		<div class="input-group" style="text-align: center">
			...
			foreach ($listeVehicule as $uneLigne) {
				...
				echo '<option selected value="'.$uneLigne['ImmatriculationVehicule'].'">'.$uneLigne['ImmatriculationVehicule']. ' - ' . $uneLigne['ConstructeurVehicule'] .' - ' . $uneLigne['ModeleVehicule'] . ' - ' . $uneLigne['CouleurVehicule'] . ' - ' . $uneLigne['CarburantVehicule'] . '</option>';
					...
					echo '<option value="'.$uneLigne['ImmatriculationVehicule'].'">'.$uneLigne['ImmatriculationVehicule']. ' - ' . $uneLigne['ConstructeurVehicule'] .' - ' . $uneLigne['ModeleVehicule'] . ' - ' . $uneLigne['CouleurVehicule'] . ' - ' . $uneLigne['TypeCarburantVehicule'] .' - ' . $uneLigne['TypeVehicule'] . '</option>';
			}
			?>
			</select>
		</div>
