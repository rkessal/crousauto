...
<html>
<head>
	<title>Modifier un paiement d'assurance</title>

  	<?php
	require_once('bdd.php');
	include("menu.php");
	$id = $_SESSION['ImmatSaisi'];
	if(isset($_POST['DateAssurance']))
		$_SESSION['DateAssurance'] = $_POST['DateAssurance'];
	$conectBDD = new BDD();
	$listeConducteur = $conectBDD->Conducteur_Liste_ParUser($_SESSION['NoUtilisateur']);
	$infosAssurance = $conectBDD->Assurance_Liste_parDateEtImmat($id, $_SESSION['DateAssurance']);
	$InfosVehicule = $conectBDD->Vehicule_Infos_parId($id);
	//var_dump($infosAssurance);
	?>
</head>
...
<h1>Modifier un paiement d'assurance</h1>
<br>
	<?php
	if(isset($_POST["Valider"]))
	{
		...
		$conectBDD = new BDD();
		$res = $conectBDD->Assurance_Modif($id, $_SESSION['DateAssurance'], $_POST['nom'], $_POST['montant']);

		if($res)
		{
			?>
			<div class="alert alert-success" role="alert">
			  	Le paiement d'assurance du véhicule à bien été modifié.
			</div>
...
?>
		<div class="input-group" style="text-align: center">
  			...
  			<input type="date" required name="date" style="width: 200px" class="input-text form-control" aria-describedby="basic-addon1"  <?php  echo' value="' . $infosAssurance['DateAssurance']  . '"'; ?> disabled>
  		</div>
		<br>
		<div class="input-group" style="text-align: center">
  			<span class="input-group-addon" style="width: 210px" id="basic-addon1">Nom Assurance*</span>
  			<input type="text"  <?php  echo' value="' . $infosAssurance['NomAssurance']  . '"'; ?> name="nom" style="width: 200px" class="input-text form-control" aria-describedby="basic-addon1">
  		</div>
		<br>

		<div class="input-group" style="text-align: center">
  			<span class="input-group-addon" style="width: 210px" id="basic-addon1">Montant Payé (en €)*</span>
  			<input type="text" name="montant" style="width: 200px"  <?php  echo' value="' . $infosAssurance['MontantAssurance']  . '"'; ?> class="input-text form-control" aria-describedby="basic-addon1">
  		</div>
		<br>
